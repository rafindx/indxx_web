...
$conn = mysqli_connect("localhost", "indxx_demo", "NA2;G+^}hcge", "indexl_work");
function get_indices($request){
	global $conn;
	//$conn = mysqli_connect("162.209.99.233", "remoteUser", "remoteuser", "indexl_work");
	...
	$res = mysqli_query($conn, $sql) or die("er1".mysqli_error($conn));
	...
	$date_res = mysqli_query($conn, $date_query) or die("er".mysqli_error());
	...
	if (!empty($date)) 
	{
		...
		$result = mysqli_query($conn, $query) or die("er".mysqli_error());
		if(!empty($result)) 
		{
			// echo "sapna";
			$data=array();
...
function getIndexDescription($indxx_id){
	...
	$result = mysqli_query($conn, "select * from index_description where indxx_id='".$factsheet_id."'") or die("error".mysqli_error());
...
function getIndexTopConstituents($indxx_id)
{
	...
	$result = mysqli_query($conn, "select isin,name,weight,date from tbl_constituents where indxx_id='".$factsheet_id."' order by weight desc limit 5")or die("error".mysqli_error());
...
function getIndexTopTenConstituents($indxx_id)
{
	...
	$result = mysqli_query($conn, "select isin,name,weight,date from tbl_constituents where indxx_id='".$factsheet_id."' order by weight desc limit 60")or die("error".mysqli_error());
...
function getIndexCharacteristics($indxx_id){
	...
	$result2 = mysqli_query($conn,"select count(*) as totalconstituents from tbl_constituents where  indxx_id='$factsheet_id' ");
...
function getIndexRiskReturns($indxx_id){
	...
	global $conn;
	$factsheet_id = $indxx_id;
	$result = mysqli_query($conn,"select benchmark_id from indxx where id='".$factsheet_id."'");
	while($row = mysqli_fetch_assoc($result)){
		$benchmarkID = $row;
	}		
	$benchmarkID=$benchmarkID['benchmark_id'];
	$result2 = mysqli_query($conn, "select date from indxx_values where indxx_values.indxx='".$factsheet_id."' order by indxx_values.date asc limit 0,1");
	while($row = mysqli_fetch_assoc($result2)){
	...
	$result3 = mysqli_query($conn,"select date from indxx_values where indxx_values.indxx='".$factsheet_id."' order by indxx_values.date asc");
	...
	$result4 = mysqli_query($conn,"select tbl_benchmark_index_value_b.date,tbl_benchmark_index_value_b.value from tbl_benchmark_index_value_b left join indxx_values on indxx_values.date=tbl_benchmark_index_value_b.date where tbl_benchmark_index_value_b.benchmark_id='".$benchmarkID."' and indxx_values.indxx='".$factsheet_id."' and tbl_benchmark_index_value_b.date>='".$firstindexvaluedate."' order by date asc");
	
	while($row = mysqli_fetch_assoc($result4)){
		$benchmarkValuesArray[] = $row;
	}
	
	$result5 = mysqli_query($conn,"select value from tbl_benchmark_index_value_b where tbl_benchmark_index_value_b.benchmark_id='".$benchmarkID."' and date='".$firstindexvaluedate."'");
 ...
 //    print_r($data);
	...
	$total = count($data);
        $count = '0';
mysqli_query($conn,"DELETE FROM indxx_risk_return_statistics WHERE indxx_id=$indxx_id");
	...
	//echo $statistic."<br>";
        if($qtd=='')
        {
    $add = mysqli_query($conn,"INSERT INTO `indxx_risk_return_statistics` (`id`, `indxx_id`, `statistic`, `qtd`, `ytd`, `1year`, `3year`, `sbd`) VALUES (NULL, '$indxx_id', '$statistic', '0.00', '$ytd', '$oneyear', '$threeyear', '$SinceInception')");
		}
		else
		{
	$add = mysqli_query($conn,"INSERT INTO `indxx_risk_return_statistics` (`id`, `indxx_id`, `statistic`, `qtd`, `ytd`, `1year`, `3year`, `sbd`) VALUES (NULL, '$indxx_id', '$statistic', '$qtd', '$ytd', '$oneyear', '$threeyear', '$SinceInception')");
...
//			echo ("<script LANGUAGE='JavaScript'> window.alert('Risk & Return Added Successfully'); </script>");
 ...
 if ($count == $total) {
    ...
    window.location.href='https://www.indxx.com/admin/calculate';
    </script>");
	}
...
function getMaxDate($factsheet_id){
	...
	$result = mysqli_query($conn,"select date from indxx_values where indxx='$factsheet_id'  order by date DESC limit 0,1");
	while($row = mysqli_fetch_assoc($result)){
		$data[] = $row;
	}
...
function getOldDate($factsheet_id,$newDate,$yearback,$return=""){
	...
	global $conn;
	$result = mysqli_query($conn,'select date from indxx_values where indxx='.$factsheet_id.' and  date <= DATE_SUB("'.$newDate.'",INTERVAL '.$yearback.' YEAR) order by date desc limit 0,1');
	while($row = mysqli_fetch_assoc($result)){
		$data[] = $row;
	}
...
function get52WeaksMinmax($factsheet_id){
	...
	$result = mysqli_query($conn,"select max(value) as maximumvalue from indxx_values where indxx='$factsheet_id' and date>='$oldDate' order by date asc");
	while($row = mysqli_fetch_assoc($result)){
		$data[] = $row;
	}

	$result2 = mysqli_query($conn,"select min(value) as minvalue  from indxx_values where  indxx='$factsheet_id' and date>='$oldDate' order by date asc");
	while($row = mysqli_fetch_assoc($result2)){
		$data2[] = $row;
	}
...
function getQuarterCorrel($factsheet_id,$benchmarkIVArray){
	...
	foreach($QTDBenchmarkDatesArray as $key=>$benchmarkV)
	{
		...
		if(!empty($benchmarkIVArray[$date]))
		echo '<br>'.$oldBValue=$benchmarkIVArray[$date];
		else{
			if($key==0)
