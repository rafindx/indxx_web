...
class DbUtils extends mysqli {
    ...
    public function __construct() {
        $this->conn = mysqli_connect("localhost", "mantis_indxxci", "u2+aAG#_v~98", "mantis_indxxcinew");
    //    $sql = mysqli_query($this->conn,"select `id` from `indxx`");
	   // $res = mysqli_query($this->conn, $sql) or die("er1".mysqli_error($this->conn));
	    print_r($res);

	    $result = mysqli_query($this->conn,"select `id` from `indxx`");
...
function getIndexRiskReturns($indxx_id){
 ...
 //        die();
    ...
    // 	(NULL, '$indxx_id', '$statistic', '$qtd', '$ytd', '$oneyear', '$threeyear', '$SinceInception')");
		...
		$update = mysqli_query($this->conn,"UPDATE `indxx_risk_return_statistics` SET `statistic`= '".$statistic."' WHERE `indxx_id` = '".$indxx_id ."'");
		if($update)
		{
			$update2 = mysqli_query($this->conn,"UPDATE `indxx_risk_return_statistics` SET `qtd`= '".$qtd."' WHERE `indxx_id` = '".$indxx_id ."'");
            
            if($update2)
            {
            	$update3 = mysqli_query($this->conn,"UPDATE `indxx_risk_return_statistics` SET `ytd`= '".$ytd."' WHERE `indxx_id` = '".$indxx_id ."'");
                if($update3)
                {
                	$update4 = mysqli_query($this->conn,"UPDATE `indxx_risk_return_statistics` SET `1year`= '".$oneyear."' WHERE `indxx_id` = '".$indxx_id ."'");
                    if($update4)
                    {
                    	$update5 = mysqli_query($this->conn,"UPDATE `indxx_risk_return_statistics` SET `3year`= '".$threeyear."' WHERE `indxx_id` = '".$indxx_id ."'");
                        if($update5)
                        {
                        	$update6 = mysqli_query($this->conn,"UPDATE `indxx_risk_return_statistics` SET `sbd`= '".$SinceInception."' WHERE `indxx_id` = '".$indxx_id ."'");
                            if($update6)
                            {
 ...
 //    window.location.href='http://mantissystems.com/Clients/indxxci/index.php/admin/calculate';
 //    </script>");
	// }
